# Matar next dev E next start
$procs = Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -like '*next dev*' -or $_.CommandLine -like '*start-server*' }
foreach ($p in $procs) {
    Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue
}
Write-Output ('parados: ' + @($procs).Count)
Start-Sleep -Seconds 2

# Build de producao (necessario para next start)
Write-Output 'buildando...'
& "C:\Users\Home\AppData\Local\hermes\node\node.exe" "C:\DevCoder\controle-popular\node_modules\next\dist\bin\next" build --webpack 2>&1 | Out-Null
Write-Output 'build OK'

# Iniciar next start (producao — SEM eval, SEM WebSocket HMR)
Start-Process -FilePath "C:\Users\Home\AppData\Local\hermes\node\node.exe" `
  -ArgumentList "C:\DevCoder\controle-popular\node_modules\next\dist\server\lib\start-server.js","start","-p","3000" `
  -WorkingDirectory "C:\DevCoder\controle-popular\apps\web" `
  -WindowStyle Hidden
Write-Output 'next start reiniciado — aguardando 10s para warmup...'
Start-Sleep -Seconds 10
Start-Process -FilePath "C:\Users\Home\AppData\Local\hermes\node\node.exe" `
  -ArgumentList "C:\DevCoder\controle-popular\node_modules\tsx\dist\cli.mjs","C:\DevCoder\controle-popular\scripts\warmup-dev.mts","3000" `
  -WorkingDirectory "C:\DevCoder\controle-popular\apps\web" `
  -WindowStyle Hidden
Write-Output 'warmup em background'

...
def _adapt(v):
    ...
    `desembargadores.raw`) sozinho — precisa do wrapper `Json`. Arrays de
...
class _QueryBuilder:
    ...
    outros. Não implementa `.or_()`, `.contains()`, `.single()` nem
    `.ilike()`: nenhum módulo usa, e um stub adivinhado da sintaxe do
    PostgREST erraria em silêncio. Quem chamar quebra com AttributeError na
    cara — não com dado errado.
    """
            ...
            with self._conn.cursor(row_factory=dict_row) as cur:
                if self._op in (None, "select"):
                    ...
                    if self._count_mode:
                        ...
                        # limit/offset; uma segunda query é o equivalente
                        # honesto. Devolver len(rows) mentiria sempre que
                        # houvesse paginação.
                        cparams: list = []
...
def fetch_all(query_factory, page_size: int = PAGE_SIZE) -> list[dict]:
    ...
    página gigante numa tabela que cresce. O volume deste eixo é grande
    desde o primeiro dia, então paginar é o caminho padrão, não otimização.

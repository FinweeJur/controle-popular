...
import { ROTULOS, descreverArea, textoParaPedido } from './rotulos.js';
...
/** Uma linha de dado, já com as colunas derivadas de área. */
...
export function paraCsv(entradas, ficticias = [], agora = new Date()) {
  const usadas = COLUNAS.filter(([chave]) =>
    ...
    || entradas.some((e) => {
      ...
      return p[chave] !== undefined && p[chave] !== null && p[chave] !== '';
    }));
...
/** Nome do arquivo: diz o que é, quantas áreas e de quando. */
export function nomeDoArquivo(formato, n, agora = new Date()) {
  return `terras-${n}-areas-${hoje(agora)}.${FORMATOS[formato].ext}`;
}
...
export function exportar(formato, entradas, { baixar = baixarNoNavegador, agora = new Date() } = {}) {
  ...
  const nome = nomeDoArquivo(formato, exportaveis.length, agora);

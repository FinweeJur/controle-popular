...
import { fetchAtosDiario } from "@/lib/betim/diario";
import { paramsDasCidades } from "@/lib/betim/staticParams";
import { obterCidadePorSlug } from "@/lib/db/queries/municipios";

/**
 * Índice estático fatiado de `/[municipio]/prefeitura/diario` —
 * gera fatias de até 2 MiB (orçamento padrão) servindo manifesto.json e fatias 0.json, 1.json...
 */
...
async function arquivos(municipioSlug: string): Promise<ArquivoIndice[]> {
  ...
  if (!pendente) {
    pendente = (async () => {
      ...
      const rows = await fetchAtosDiario(cidade.id_municipio);

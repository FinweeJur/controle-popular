...
export default function AdminPage() {
    ...
    async (t: string) => {
      ...
      try {
        const [modRes, adsRes] = await Promise.all([
          fetch(withBasePath("/api/admin/moderacao"), { headers: authHeaders(t) }),
          fetch(withBasePath("/api/admin/anuncios"), { headers: authHeaders(t) }),
        ]);
  ...
  async function handleModerar(tabela: string, id: string, aprovado: boolean) {
    await fetch(withBasePath("/api/admin/moderacao"), {
      method: "POST",
  ...
  async function handleCriarAnuncio(e: React.FormEvent<HTMLFormElement>) {
    ...
    await fetch(withBasePath("/api/admin/anuncios"), {
      method: "POST",
      headers: authHeaders(token),
      body: JSON.stringify({
  ...
  async function handleToggleAtivo(id: string, ativo: boolean) {
    await fetch(withBasePath(`/api/admin/anuncios/${id}`), {
      method: "PATCH",
      headers: authHeaders(token),

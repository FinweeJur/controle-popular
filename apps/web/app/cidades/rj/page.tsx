...
export default async function PageCidadesRJ() {
  // Verifica se os dados foram publicados via build (public/municipios/rj/)
  const publicDir = path.join(process.cwd(), "apps/web/public/municipios/rj");
  let base = "/municipios/rj";
  let total = 0;
  ...
  if (fs.existsSync(path.join(publicDir, "manifesto.json"))) {
    ...
    const dataDir = path.join(process.cwd(), "apps/web/data", "municipios-rj.json");
  ...
  const cartao = {
    titulo: "Municípios de RJ",
    valor: total.toLocaleString("pt-BR"),
    descricao: "Cidades no Rio de Janeiro mapeadas pelo IBGE",
  };
    ...
    <article className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-8">
        <nav aria-label="Você está em" className="text-sm text-text_soft">
          <Link href="/">Controle Popular</Link> {" > "} <span>Cidades</span> {" > "} <span>RJ</span>
        </nav>
        <h1 className="mt-2 text-3xl font-bold">Municípios do Rio de Janeiro</h1>
        <p className="mt-3 text-text_soft">
          Todos os 92 municípios do Rio de Janeiro mapeados via API do IBGE. Dados atualizados
          no build — <a href="https://servicodados.ibge.gov.br/api/v1/localidades/estados/33/municipios" target="_blank" rel="noopener noreferrer">Fonte</a>.
        </p>
      </header>

...
export default async function PageCidadesBA() {
  // Verifica se os dados foram publicados via build (public/municipios/ba/)
  const publicDir = path.join(process.cwd(), "apps/web/public/municipios/ba");
  let base = "/municipios/ba";
  let total = 0;
  ...
  if (fs.existsSync(path.join(publicDir, "manifesto.json"))) {
    ...
    const dataDir = path.join(process.cwd(), "apps/web/data", "municipios-ba.json");
  ...
  const cartao = {
    titulo: "Municípios de BA",
    valor: total.toLocaleString("pt-BR"),
    descricao: "Cidades na Bahia mapeadas pelo IBGE",
  };
    ...
    <article className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-8">
        <nav aria-label="Você está em" className="text-sm text-text_soft">
          <Link href="/">Controle Popular</Link> {" > "} <span>Cidades</span> {" > "} <span>BA</span>
        </nav>
        <h1 className="mt-2 text-3xl font-bold">Municípios da Bahia</h1>
        <p className="mt-3 text-text_soft">
          Todos os 417 municípios da Bahia mapeados via API do IBGE. Dados atualizados
          no build — <a href="https://servicodados.ibge.gov.br/api/v1/localidades/estados/29/municipios" target="_blank" rel="noopener noreferrer">Fonte</a>.
        </p>
      </header>

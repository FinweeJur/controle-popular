...
import atosPorMunicipio from "../../data/diario-atos-municipios.json";
...
export async function getAtosDiarioMunicipio(
  ...
  const dataset = atosPorMunicipio as unknown as Record<string, AtoDiarioSumario[]>;
  return dataset[slug] ?? dataset["diamantina"] ?? [];
}

 ...
 *
 * O projeto Supabase é COMPARTILHADO com o /betim e o /congresso, e os schemas
 ...
 * recusar o cliente configurado para `judiciario`. Inferir do `criar()`
 * mantém o schema no tipo e faz o compilador acompanhar a lib se a
 * assinatura dos genéricos mudar de versão.
 */
export type ClienteJudiciario = ReturnType<typeof criar>;
 ...
 */
export function getSupabaseClient(): ClienteJudiciario | null {
 ...
 * component.
 */
export function getSupabaseServiceClient(): ClienteJudiciario | null {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;

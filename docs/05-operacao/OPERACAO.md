...
3. Varra dado pessoal **antes de commitar** dado coletado: `apps/web/lib/sem-cpf-no-repo.test.ts` valida por mod-11 todo campo de texto em código/doc, e `scripts/checar-dado-pessoal-em-dado.py` varre o DADO ingerido (JSON de `apps/web/data/` e `docs/dados/` — e `--extra` para dump novo, ex. o do Solr da Brumadinho); ambos rodam no pre-push e na CI. CPF em ementa oficial é redigido na própria ingestão.

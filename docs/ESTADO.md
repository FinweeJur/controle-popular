...
1. **Restaurar o ETL da FGV** — `git restore etl/betim/etl/apis/__init__.py etl/betim/etl/apis/fgv_paraopeba.py`. Os dois estão apagados no diretório de trabalho, **sem commit e sem decisão**, e `.github/workflows/etl-betim.yml:359` ainda chama o módulo: commitar assim para a tela de Betim (`paraopeba_saldo_municipio`, `paraopeba_iniciativas`) de atualizar. Custo: um comando.
2. **Diário oficial D1** — coletor SIGPub (Diamantina), com o corte da decisão 1. Migration `0077` e classificador (`lib/diario/classificarAto.ts`, 67/70 títulos reais) já estão prontos.
3. **Itinga no `diario_oficial`** — duas linhas em `apps/web/lib/db/cidades-do-build.ts` (é onde as outras 5 cidades declaram a fonte) + o seed equivalente. A rota `[municipio]/prefeitura` só renderiza o card quando o campo existe, então hoje Itinga não tem card nenhum.
...
6. **Chatbot, passo 1: fechar a peça que falta.** ⚠️ Medido em 22/08: **nem Maritaca nem DeepSeek publicam endpoint de embeddings** (a doc da Maritaca recomenda a DeepInfra; a do DeepSeek só documenta `chat/completions`). RAG precisa de embeddings antes de geração — decidir o vetorizador antes de escrever código. Caminho de custo zero: vetorizar **local no home-pc** (bge-m3 ou e5-large via Ollama), que também mantém o texto na máquina até a varredura de dado pessoal passar. Depois: provar conceito com **1 documento** (chunk → vetor → pergunta → resposta com citação), como o plano manda.

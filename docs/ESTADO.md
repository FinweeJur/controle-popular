...
6. **Chatbot, passo 1: fechar a peça que falta.** ⚠️ Medido em 22/08: **nem Maritaca nem DeepSeek publicam endpoint de embeddings** (a doc da Maritaca recomenda a DeepInfra; a do DeepSeek só documenta `chat/completions`). RAG precisa de embeddings antes de geração — decidir o vetorizador antes de escrever código. Caminho de custo zero: vetorizar **local via Ollama**, que também mantém o texto na máquina até a varredura de dado pessoal passar. ✅ **Medido em 22/08 nesta máquina:** o Ollama já tem `nomic-embed-text` instalado e ele devolve **768 dimensões** para uma frase de teste (5,5 s na primeira chamada, com o modelo frio) — e é exatamente a dimensão que o índice de código do `code-graph-rag` já usa (`.qdrant_code_embeddings/meta.json`: `size: 768`, distância Cosine). Ou seja, a metade "vetorizar" do RAG **já roda aqui**, sem cartão e sem terceiro; o que falta contratar é só a metade "escrever". Depois: provar conceito com **1 documento** (chunk → vetor → pergunta → resposta com citação), como o plano manda.

...
- **Testes**: `npm test` na raiz (delega ao workspace e roda os dois: vitest e os testes do globo em `node:test`). Medido em 21/08/2026: **57 arquivos, 776 testes no vitest + 137 no globo**. Typecheck: `npx tsc --noEmit`. Número de teste sem data ao lado já circulou errado em seis versões neste repo — ver `docs/DESENVOLVIMENTO.md`, seção "Verificação".

...
- [Propósito](#propósito)
- [1. Os dois arquivos que já existem](#1-os-dois-arquivos-que-já-existem)
- [2. `cfem-municipios.geojson` — entrada no `LAYER_REGISTRY`](#2-cfem-municipiosgeojson-entrada-no-layer-registry)
- [3. `cruzamento-dinheiro-ambiental-4cidades.geojson` — entrada no `LAYER_REGISTRY`](#3-cruzamento-dinheiro-ambiental-4cidadesgeojson-entrada-no-layer-registry)
- [4. Os números por trás dos dois arquivos, medidos em 13/08/2026](#4-os-números-por-trás-dos-dois-arquivos-medidos-em-13082026)

## Propósito

Este documento é para quem estiver com `apps/web/public/terras/globo/js/config.js` e `js/ui/layerspanel.js` abertos — **eu não toquei em nenhum dos dois**, de propósito: outro agente estava editando os dois arquivos ao vivo, noutro worktree, no momento desta entrega, e mexer neles teria dado conf...

...
- [Propósito](#propósito)
- [Universo varrido (por inteiro, não amostra)](#universo-varrido-por-inteiro-não-amostra)
- [Método: dois motores, e verificação de CONTEÚDO, não só status](#método-dois-motores-e-verificação-de-conteúdo-não-só-status)
- [Resultado por tabela](#resultado-por-tabela)
- [Arquivamento — medição real (Tarefa 2)](#arquivamento-medição-real-tarefa-2)
- [O que NÃO foi coberto por esta rodada](#o-que-não-foi-coberto-por-esta-rodada)
- [Verificação de segurança e qualidade](#verificação-de-segurança-e-qualidade)

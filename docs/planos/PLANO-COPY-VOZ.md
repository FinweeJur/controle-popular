# Proposta de copy — "O povo pergunta. E também canta."
...
- [Bloco "Já aconteceu aqui" nas cidades](#bloco-já-aconteceu-aqui-nas-cidades-municipio)
- [`/sobre` — manifesto longo](#sobre----manifesto-longo)
- [Microcopy de estado](#microcopy-de-estado)
...
A voz é a de **alguém que conhece a história do país e senta do seu lado para olhar a tela junto**: explica sem dar aula, faz graça da papelada — nunca das pessoas — e termina cada capítulo entregando uma ferramenta de verdade. A resistência mora na memória ("o povo sempre perguntou"); a esperança mora no presente ("agora a resposta existe"); o humor mora no alívio ("e nem precisa sair de casa").

Trilhos que não se cruzam, herdados do rebrand e do AGENTS.md:

1. **O grito mora no letreiro; o número mora com a fonte.** Resistência e leveza no editorial (hero, capítulos, marquee, manifesto); sobriedade total dentro de card de número, ressalva e tabela.
...
> Informação oficial que sempre foi pública — só estava espalhada por dezenas de sistemas, escrita em dialeto de edital. A gente juntou tudo numa tela, por cidade e por tema, e traduziu para português comum. Cada número diz de onde saiu.

**CTA duplo:** `Ver minha cidade` · `Escolher uma frente`

**Label do count-up:** `R$ [total] públicos de olho — e contando.` + `FONTE: · atualizado em {data}`
...
**Nota de régua (subtexto "como classificamos"):**

> Em 1874, o sertão quebrou os quilos que o Império impôs sem perguntar a ninguém — a lição ficou: medida boa é a que todo mundo pode conferir. A nossa régua é pública, cada direito tem âncora legal e qualquer pessoa audita item a item.
...
## Bloco "Já aconteceu aqui" nas cidades (`/[municipio]`)

Cartão-carimbo no painel da cidade (chrome, não é card de dado), com três linhas: **memória** (o que já ocorreu lá, com ano), **cultura viva** (o que continua acontecendo) e **ponte** (a ligação com o painel). Tom de orgulho de vizinhança — nunca de museu. A copy vive em `apps/web/lib/memoria-cidades.ts`.
...
- **Lacuna de dados:** `A FONTE NÃO TEM ESSE DADO.` → *A gente registra a ausência, com data. Lacuna escondida vira surpresa — e surpresa, aqui, não.*
- **404:** *Esta página não existe. Se fosse contrato sem licitação, a gente achava.*
- **Botão CSV:** `Baixar planilha` → tooltip: *O dado é seu. Pode levar.*
- **Ofício em PDF:** `Gerar ofício` → *Pronto para preencher. Leva menos de um minuto.*
- **Seletor de temas:** *Escolha a sua candeia.*
- **Badge de alerta:** `⚠ ALERTA — sinal de investigação, não veredito` *(a distinção violação legal × heurística é inegociável)*
- **Rodapé de card de número:** `FONTE: {fonte} · {data}` *(permanece em todo card, inclusive nos capítulos)*
